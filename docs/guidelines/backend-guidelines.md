...
配置文件路径按命令行 `-config`、`CONFIG_FILE`、默认 `config.json` 的顺序选择。最终选中的文件不存在时保留内置开发默认值，不继续回退到其他文件；环境变量随后覆盖已加载值，命令行 `-port` 最后覆盖服务端口。

服务端口的优先级为：命令行 `-port`、环境变量 `SERVER_PORT`、配置文件 `server.port`、内置默认值 `8080`。命令行端口必须是 `1` 至 `65535` 的整数。
