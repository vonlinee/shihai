...
// AdminCreateUser 管理员创建用户（支持指定角色）
func (s *UserService) AdminCreateUser(req *dto.AdminCreateUserRequest) (*dto.UserResponse, error) {
	// 检查用户名是否已存在
	_, err := s.userRepo.GetByUsername(req.Username)
	if err == nil {
		return nil, errors.New("username already exists")
...
func (s *UserService) toUserResponse(user *models.User) *dto.UserResponse {
	...
	// 设置主角色（优先级：admin > editor > reviewer > user）
	role := "user"
	for _, r := range roles {
		if r == "admin" {
			role = r
		...
		if r == "editor" && role != "admin" {
			role = r
		}
		if r == "reviewer" && role != "admin" && role != "editor" {
			role = r
		}
	}

	// 确保 roles 不为 nil，避免 JSON 序列化为 null
	if roles == nil {
		roles = []string{}
	}
