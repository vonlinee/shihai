...
type CommentCreateRequest struct {
	PoemID      RequestID  `json:"poemId" binding:"required"`    // 评论所属诗词ID，必填
	Content     string     `json:"content" binding:"required"`   // 评论正文内容，必填
	ParentID    *RequestID `json:"parentId"`                     // 父评论ID，回复评论时传入，顶级评论不传
...
type CommentResponse struct {
	ID          uint64            `json:"id,string"`         // 评论ID
	PoemID      uint64            `json:"poemId,string"`     // 所属诗词ID
	UserID      *uint64           `json:"userId,string"`     // 评论用户ID，已登录用户有值
	...
	ParentID    *uint64           `json:"parentId,string"`   // 父评论ID
...
type CommentVoteRequest struct {
	CommentID RequestID `json:"commentId" binding:"required"`               // 被投票的评论ID
