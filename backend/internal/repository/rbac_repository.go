...
// 注意：此处必须使用 Unscoped 进行物理删除，否则软删除留下的 deleted_at 行
// 仍占用 (role_id, code) 唯一索引，会导致后续重新分配权限时插入冲突。
func (r *RolePermissionRepository) DeleteByRoleID(roleID uint64) error {
	return r.db.Unscoped().Where("role_id = ?", roleID).Delete(&models.RolePermission{}).Error
}
...
func (r *UserRoleRepository) Delete(userID, roleID uint64) error {
	return r.db.Unscoped().Where("user_id = ? AND role_id = ?", userID, roleID).Delete(&models.UserRole{}).Error
}

// DeleteByUserID 删除用户的所有角色
// 注意：此处必须使用 Unscoped 进行物理删除，否则软删除留下的 deleted_at 行
// 仍占用 (user_id, role_id) 唯一索引，会导致后续重新分配角色时插入冲突。
func (r *UserRoleRepository) DeleteByUserID(userID uint64) error {
	return r.db.Unscoped().Where("user_id = ?", userID).Delete(&models.UserRole{}).Error
}
...
func (r *PermissionRepository) List(page, pageSize int, module string) ([]models.Permission, int64, error) {
	...
	offset := (page - 1) * pageSize
	if err := query.Offset(offset).Limit(pageSize).Order("module, code").Find(&perms).Error; err != nil {
		return nil, 0, err
	}
