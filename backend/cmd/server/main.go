...
func initRBACData(app *App) error {
	...
	if err != nil {
		...
		if err == nil {
			userRoleRepo.Create(&models.UserRole{
				UserID: rootUser.ID,
				RoleID: adminRole.ID,
			})
		}
...
func setupRoutes(r *gin.Engine, app *App) {
	api := r.Group("/api")
	{
		...
		admin.Use(middleware.Auth(), app.rbacMiddleware.RequireAnyRole("admin", "editor"))
		{
			...
			admin.POST("/users", app.rbacMiddleware.RequirePermission("user:create"), app.userHandler.AdminCreateUser)
			...
			// Dynasties & Poets
			admin.POST("/dynasties", app.rbacMiddleware.RequirePermission("poem:create"), app.poemHandler.CreateDynasty)
			admin.PUT("/dynasties/:id", app.rbacMiddleware.RequirePermission("poem:update"), app.poemHandler.UpdateDynasty)
			admin.DELETE("/dynasties/:id", app.rbacMiddleware.RequirePermission("poem:delete"), app.poemHandler.DeleteDynasty)
			admin.POST("/poets", app.rbacMiddleware.RequirePermission("poem:create"), app.poemHandler.CreatePoet)
			admin.PUT("/poets/:id", app.rbacMiddleware.RequirePermission("poem:update"), app.poemHandler.UpdatePoet)
			admin.DELETE("/poets/:id", app.rbacMiddleware.RequirePermission("poem:delete"), app.poemHandler.DeletePoet)
