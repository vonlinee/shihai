...
func setupRoutes(r *gin.Engine, app *App) {
	api := r.Group("/api")
	{
		...
		admin.Use(middleware.Auth(), app.rbacMiddleware.RequireAnyRole(models.RoleAdmin, models.RoleEditor))
		{
			...
			admin.GET("/ci-tunes", app.rbacMiddleware.RequirePermission(models.PermPoemList), app.poemHandler.GetCiTunes)
			admin.POST("/ci-tunes", app.rbacMiddleware.RequirePermission(models.PermPoemCreate), app.poemHandler.CreateCiTune)
			admin.POST("/ci-tunes/parse-title", app.rbacMiddleware.RequirePermission(models.PermPoemList), app.poemHandler.ParseCiTuneTitle)
			admin.PUT("/ci-tunes/:id", app.rbacMiddleware.RequirePermission(models.PermPoemUpdate), app.poemHandler.UpdateCiTune)
			admin.DELETE("/ci-tunes", app.rbacMiddleware.RequirePermission(models.PermPoemDelete), app.poemHandler.BatchDeleteCiTunes)
			admin.DELETE("/ci-tunes/:id", app.rbacMiddleware.RequirePermission(models.PermPoemDelete), app.poemHandler.DeleteCiTune)
			...
			admin.POST("/poems/pingze-recognition", app.rbacMiddleware.RequirePermission(models.PermPoemUpdate), app.pingzeHandler.RecognizePingze)
